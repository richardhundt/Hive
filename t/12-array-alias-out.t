...

{
    ...
    sub execute {
        ...
        do {
            $self->output->at(0)->put( "tick $count" );
            $self->output->at(1)->put( "tick $count" );
            $self->output->at(2)->put( "tick $count" );
        } while ++$count < 10;
...
$gen->output->at(0)->to( $log0->intake );
$gen->output->at(1)->to( $log1->intake );
$gen->output->at(2)->to( $log2->intake );
